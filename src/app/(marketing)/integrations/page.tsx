...
const BUILT: { name: string; detail: string; env?: string; href?: string }[] = [
  ...
  { name: 'SMTP', detail: 'Any server. Mail drains from a transactional outbox via a worker, with retries and a delivery log.', env: 'MAIL_DRIVER=smtp' },
  ...
  { name: 'Meilisearch', detail: 'Or PostgreSQL full-text, behind one interface, kept current by incremental sync.', env: 'SEARCH_DRIVER=meilisearch' },
  { name: 'OpenAI, Azure OpenAI, Anthropic', detail: 'Course-outline and lesson drafting. Disabled by default, and while disabled the feature is hidden rather than broken.', env: 'AI_PROVIDER=disabled' },
...
const NOT_BUILT: { name: string; detail: string }[] = [
  {
    ...
    detail:
      'Zoom, Meet, Teams and Jitsi. No adapter — you paste the link onto the session, and attendance is recorded here rather than synced from the provider.',
  },
  {
  ...
  { name: 'Video transcoding', detail: 'The storage port and player are built; the transcoding worker is not. Upload what a browser can play.' },
  ...
  return (
    <>
      ...
      <PageHero
        ...
        lead="No logo wall. A logo grid implies partnerships we do not have, and it is always missing the second column — the one you need before you commit."
        trail={TRAIL}
      />
      ...
      <Act labelledBy="built-heading" surface="paper" spacing="normal">
        <div className="container-mk">
          ...
          <p className="mt-4 max-w-measure text-mk-body text-fg-muted">
            Most are <strong className="text-fg">ports with drivers</strong> rather than
      ...
      <Act labelledBy="not-built-heading" surface="muted" spacing="normal">
        <div className="container-mk">
          ...
          <p className="mt-4 max-w-measure text-mk-body text-fg-muted">
            Printing this column is the point of the page. One of these is probably a dealbreaker
            for somebody reading, and it is cheaper for both of us if they find out here.
          </p>
          ...
          <p className="mt-block max-w-measure text-mk-body text-fg-muted">
            We would rather list <Counter value={BUILT.length} /> real integrations than sixty
            logos we have a screenshot of.{' '}
      ...
      <ClosingCTA
        ...
        lead="Then the fastest next step is the sandbox, or twenty minutes with somebody who can answer follow-ups."
