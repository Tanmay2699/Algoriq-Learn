...
import { Counter, Heading, Mono } from '../../../components/primitives';
...
const SUMMARY: { control: string; how: React.ReactNode; where: string }[] = [
  {
  ...
  },
  {
    ...
    how: 'The application role is not the owner, not a superuser, holds no BYPASSRLS',
    where: 'apps/api/prisma/rls.ts',
  },
  {
  ...
  },
  {
    ...
    how: (
      <>
        Argon2id with lockout, <Counter value={15} suffix="-minute" /> access tokens, rotating
  ...
  },
  {
    ...
    how: 'httpOnly session cookie; the JWT is attached server-side and never reaches client JavaScript',
    where: 'apps/web BFF',
  },
  {
    control: 'Audit',
    ...
    <>
      ...
      <Act labelledBy="summary-heading" surface="paper" spacing="normal">
        <div className="container-mk">
          ...
          <p className="mt-4 max-w-measure text-mk-body text-fg-muted">
            For the reviewer with a checklist. Everything below expands a row.
          </p>
          <div className="mt-8">
        ...
        <div className="container-mk max-w-prose">
          ...
          <div className="mt-6 space-y-4 text-mk-body text-fg-muted">
            <p>
              ...
              policy — the most common way row-level security is deployed and does nothing.
            </p>
            <p>
              <Mono>FORCE</Mono> is not enough alone: a superuser, or any role with{' '}
              <Mono>BYPASSRLS</Mono>, ignores policies unconditionally. The application connects
              as none of those, the owner role is reserved for migrations on a separate connection
              string, and the check verifies the runtime role too.
            </p>
            <p>
              The policy set is <strong className="text-fg">derived from the database catalogue</strong>
              rather than hand-written per table, so every table with a <Mono>tenant_id</Mono>
              column is covered automatically and a gap fails CI. A policy that must be remembered
              on each new table is eventually forgotten, invisibly, until it leaks.
            </p>
            <p>
              <strong className="text-fg">
                <Counter value={114} /> tables are covered.
              </strong>{' '}
              Thirteen are exempt, and the list is published rather than denied.
            </p>
          </div>

          <h3 className="mt-block text-mk-title font-semibold text-fg">The thirteen exemptions</h3>
          <p className="mt-3 text-mk-body text-fg-muted">Two groups, two reasons.</p>
          <ul className="mt-4 space-y-3 text-mk-body text-fg-muted">
            <li>
              <strong className="text-fg">No tenant id at all</strong> — users, credentials,
              identities, permissions, tenants. A user is a global principal who may belong to
              several institutes; the <em>membership</em> is what is scoped, and it sits in the
              second group.
            </li>
            <li>
              <strong className="text-fg">The identity plane</strong> — sessions, permission
              versions, memberships, invitations, tenant domains. These carry a tenant id but are
              read <em>in order to determine</em> the tenant, before any binding exists. Refreshing
              a token means looking a session up by id to discover its institute, which a policy
              requiring the institute first makes unresolvable. Every access is keyed by an
              unguessable id.
            </li>
          </ul>
        </div>
        ...
        <div className="container-mk">
          ...
          <div className="mt-6 max-w-prose space-y-4 text-mk-body text-on-ink-muted">
            <p>
              ...
              <Mono>@Public()</Mono>. A lint rule and a coverage check fail the build otherwise,
              so &ldquo;we forgot the guard&rdquo; is not a bug that can reach production.
            </p>
            <p>
              <strong className="text-on-ink">
                <Counter value={272} /> keys, <Counter value={264} /> enforced on a route.
              </strong>{' '}
              The remaining eight belong to surfaces designed but not yet built. We publish that
              number too: unenforced keys are the normal state of a growing catalogue, and hiding
              it would be the suspicious choice.
            </p>
            <p>
              Hiding a button somebody cannot use is a courtesy, not a boundary. It is never the
              only check.
            </p>
          </div>
          ...
          <div className="mt-block">
            ...
            <p className="mt-2 max-w-measure text-mk-body-sm text-on-ink-muted">
              Every act the software can perform, with the description an administrator sees when
              building a role.
            </p>
            <div className="mt-6">
              <PermissionCatalog />
      ...
      <Act id="roles" labelledBy="roles-heading" surface="paper" spacing="normal">
        <div className="container-mk grid gap-10 lg:grid-cols-2 lg:gap-14">
          <div>
            ...
            <div className="mt-6 space-y-4 text-mk-body text-fg-muted">
              <p>
                A role is a named bundle of keys plus a scope. Eleven templates ship, and each
                institute gets editable copies. <strong className="text-fg">Nothing in the code
                branches on a role&apos;s name</strong> — a guard asks whether somebody holds a
                key, never what they are called.
              </p>
              <p>
                Scopes: global, organisation, institute, branch, department, course, batch or a
                single record. Learners and parents hold record scope, which is why a parent sees
                their own children and nothing else.
              </p>
              <p>
                On top: per-user overrides with an expiry, and out-of-office delegation. Denials
                win.
              </p>
            </div>
          </div>
      ...
      <Act id="identity" labelledBy="identity-heading" surface="muted" spacing="normal">
        <div className="container-mk max-w-prose">
          ...
          <ul className="mt-6 space-y-3 text-mk-body text-fg-muted">
            ...
            <li>TOTP multi-factor with recovery codes, and a revocable device list.</li>
            <li>Sign-in with Google, Microsoft and GitHub.</li>
            <li>
              People are added <strong className="text-fg">by invitation only</strong>. No route
              lets an administrator set somebody else&apos;s password.
            </li>
            <li>
              Impersonation for support, with dual audit attribution: the log records who acted
              and on whose behalf.
            </li>
          </ul>
        ...
        <div className="container-mk max-w-prose">
          ...
          <div className="mt-6 space-y-4 text-mk-body text-fg-muted">
            <p>
              ...
              diff and a hash of the entry before it. Tampering breaks the chain from that point
              on, detectable without trusting the application that wrote it.
            </p>
            <p>
              ...
              Erasure tombstones the record rather than rewriting the chain that proves it
              happened — the part most implementations get backwards.
            </p>
          </div>
        </div>
      </Act>
        ...
        <div className="container-mk grid gap-10 lg:grid-cols-2 lg:gap-14">
          <div>
            ...
            <p className="mt-6 max-w-measure text-mk-body text-fg-muted">
              ...
              code is usually an employer, not a user. The page renders exactly what the API
              returns — what was certified, to whom, when, whether it still stands — and nothing
              else.
            </p>
            <p className="mt-4 max-w-measure text-mk-body text-fg-muted">
              Unknown and revoked both render, in different words. &ldquo;Nothing found&rdquo; and
              &ldquo;withdrawn&rdquo; mean very different things to the person checking.
            </p>
          </div>
          <VerifyCertificate />
        </div>
      ...
      <Act id="scale" labelledBy="scale-heading" surface="paper" spacing="normal">
        <div className="container-mk max-w-prose">
          ...
          <div className="mt-6 space-y-4 text-mk-body text-fg-muted">
            <p>
              The API is horizontally scalable: the rate limiter, permission-grant cache and
              tenant host resolution sit in shared cache rather than per-process memory, and
              production refuses the in-memory driver. Rate limiting runs before the guards, and
            ...
            <p>
              ...
              throughput limit on a very large deployment, and it is on our list.
            </p>
          </div>
        </div>
      </Act>
        ...
        <div className="container-mk max-w-prose">
          ...
          <div className="mt-6 space-y-4 text-mk-body text-on-ink-muted">
            <p>
              ...
              imported in feature code.</strong> Moving between them is a connection string, not a
              project.
            </p>
            <p>
              <Mono>docker compose up</Mono> boots the whole platform on a laptop with no cloud
              account. Everything exports. The database is ordinary PostgreSQL.
            </p>
            <p>
              Not a developer convenience: it is what makes the rest of this page worth trusting.
              A vendor who has made leaving easy has fewer reasons to make staying compulsory.
            </p>
          </div>
        </div>
      </Act>
      ...
      <Act id="not-yet" labelledBy="not-yet-heading" surface="paper" spacing="normal">
        <div className="container-mk max-w-prose">
          ...
          <ul className="mt-6 space-y-3 text-mk-body text-fg-muted">
            <li>
              <strong className="text-fg">No SOC 2 certificate.</strong> The policies exist and
              the audit log produces the evidence automatically, but the audit has not happened —
              and we will not put a badge up before it is earned.
            </li>
            <li>
            ...
            <li>
              <strong className="text-fg">No formal uptime SLA</strong> — we have no telemetry to
              hold ourselves to one, and a status page that cannot go red is theatre.
            </li>
            <li>
              <strong className="text-fg">No multi-region data residency.</strong> Designed and
              deliberately not built: a region field that moves no bytes reads as a guarantee and
              is false.
            </li>
            <li>
              <strong className="text-fg">No customers yet</strong>, so no operational history.
      ...
      <Act id="disclosure" labelledBy="disclosure-heading" surface="muted" spacing="tight">
        <div className="container-mk max-w-prose">
          ...
          <p className="mt-4 text-mk-body text-fg-muted">
            ...
            . Acknowledged within two working days, updates while we fix it, and credit if you
            want it. We will not threaten you.
          </p>
      ...
      <ClosingCTA
        ...
        lead="Build status, sub-processors and the accessibility statement are one click away. Or open the sandbox and look at the software instead."
